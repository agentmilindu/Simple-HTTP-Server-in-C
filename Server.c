...
void php_cgi(char* script_path, int fd) {
 ...
 char script[1000];
 strcpy(script, "php -f ");
 strcpy(script, webroot());
 strcat(script, script_path);
 printf(stdout, script);
 file_path = popen(script, "r");
 int fno = fileno(file_path);
 if(!file_path){
  fprintf(stderr, "Could not open the file\n");
 }
 if(file_path==NULL){
  printf("Could not open popen the file\n");
 }
 char str[100000];
 int nbyte= 100000;
	 
 while (fgets(str, 99999, file_path)!=NULL ){
